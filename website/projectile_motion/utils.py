...
def projectile_motion_simulation(request):
    projetile_motion_data = ProjectileMotionData.objects.first()
        
    # Constants
    g = 9.807

    # Set the initial state vector at time t=0.
    r0 = np.array([0, projetile_motion_data.height])
    angle = math.radians(projetile_motion_data.angle)
    v0 = projetile_motion_data.velocity * np.array([math.cos(angle), math.sin(angle)])
    ...
    def differential_eq(t, u):
        ...
        r, v = np.split(u, 2)
        # Air drag force.
        Fd = -0.5 * projetile_motion_data.c_F * np.linalg.norm(v) * v
        # Gravity force.
        Fg = projetile_motion_data.mass * g * np.array([0, -1])
        # Acceleration.
        a = (Fd + Fg) / projetile_motion_data.mass
    ...
    plt.xlabel('x/m')
    plt.ylabel('y/m')
    plt.xlim(0, projetile_motion_data.xmax)
    plt.ylim(0, projetile_motion_data.ymax)
    plt.grid(True)
    plt.tight_layout()
